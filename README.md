...
```bash
git clone https://github.com/nithinrcheruku/gcp-architectrure-diagram-skills.git
cd gcp-architectrure-diagram-skills
./install.sh
```
...
2. **Reloading Skills in AGY 2.0**:
   - If Antigravity 2.0 or Antigravity IDE is already running, reload customizations to pick up the new skill:
     - Open **Settings → Customizations → Skills** and click **Reload Skills**, OR
     - Press `Cmd+Shift+P` / `Ctrl+Shift+P` and run **`Antigravity: Reload Customizations`** (or restart AGY 2.0).
3. **Verify Activation**: Confirm `google-cloud-architecture-diagram` is toggled **ON** in your **Active Skills** list under **Settings → Customizations → Skills**.
...
## 🎨 Architectural Baseline & Standards Enforced

Every diagram generated by this skill adheres to the official Google Cloud Architecture Center V2.5 Style Guide:

| Hierarchy Level | Visual Container | Fill Color | Stroke Color | Corner Radius |
| :--- | :--- | :--- | :--- | :--- |
| **Layer 1** | Google Cloud Header Band | `#1A73E8` | None | Square (`h=40`) |
| **Layer 2** | GCP Project Boundary | `#F6F6F6` | `#BDBDBD` | `rx="8"` |
| **Layer 3** | Regional VPC Network | `#ECEFF1` | `#90A4AE` | `rx="6"` |
| **Layer 4** | VPC Subnet Zone | `#EDE7F6` | `#5E35B1` | `rx="4"` |
| **Layer 5** | VPC-SC Security Perimeter | `#FCE8E6` | `#EA4335` (dashed) | `rx="4"` |

| Category Family | Fill Color | Stroke Color | Product Examples |
| :--- | :--- | :--- | :--- |
| **Compute & Serverless** | `#FFFFFF` | `#4284F3` (Blue) | Cloud Run, GKE, Compute Engine |
| **Databases & Analytics** | `#FFFFFF` | `#34A853` (Green) | AlloyDB, Cloud SQL, BigQuery, Spanner |
| **Security & WAF** | `#FCE8E6` | `#EA4335` (Red) | Cloud Armor, Model Armor, WAF |
| **KMS & Identity** | `#FEF7E0` | `#F9AB00` (Yellow) | Cloud KMS CMEK Key Rings, IAM |
...
```bash
python3 evals/scripts/validate_eval.py <eval_id> <output_directory>
```

Example:
```bash
python3 evals/scripts/validate_eval.py eval-01-explicit-agentic ./evals/files
```
